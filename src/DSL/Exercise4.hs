...
data ValueTag = Default | Custom

data A_Tagged a where
  A_Def :: Int -> A_Tagged 'Default
  A     :: Int -> A_Tagged 'Custom
...
type X_Empty    = X_Abs (A_Tagged 'Default) () [C_Labeled] '[]
type X' xss a   = X_Abs a B (NonEmpty C_Labeled) xss
data X          = forall xss. X (X' xss Int)

x_empty :: X_Empty
x_empty = X_Abs (A_Def 42) () []
...
type family ExactlyOnceConstraints field a a' b b' cs cs' :: Constraint where
  ExactlyOnceConstraints (A_Tagged 'Custom) a a' b b' cs cs' =
    (a ~ A_Tagged 'Default, b ~ b', cs ~ cs')
  ExactlyOnceConstraints B a a' b b' cs cs' =
    (a ~ a', b ~ (), cs ~ cs')
...
type MkX xss a r = X_M 0 '[] X_Empty (X' xss (A_Tagged a)) r

mkX :: MkX xss a_tag r -> X
mkX = X . Lens.over aField getA . snd . flip runReader [] . flip runIxStateT x_empty . runX_M
  where
    getA :: forall a. A_Tagged a -> Int
    getA (A_Def a) = a
    getA (A a) = a
...
x :: X
x = mkX $ do
  set aField (A 100)  -- This line can now be omitted since '_aField' has a default value.
  set bField "Hello"
  new_c 1.0
